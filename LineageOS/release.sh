...
response=$(jq -n --arg datetime $TIMESTAMP \
        ...
        --arg romtype $ROMTYPE \
        --arg size $SIZE \
        --arg url "https://sourceforge.net/projects/$PROJECT/files/lineage-${VERSION}/${FILENAME}/download" \
        --arg version $VERSION \
        '$ARGS.named'
)
wrapped_response=$(jq -n --argjson response "[$response]" '$ARGS.named')

echo "$wrapped_response" > LineageOS/$VERSION/$DEVICE.json
git add LineageOS/$VERSION/$DEVICE.json
git commit -m "LineageOS: Update autogenerated json for $DEVICE $VERSION ${DATE}/${TIMESTAMP}"
